...
%% Part I.j - run_Indy_car.p benchmark comparison
...
try
    ...
    % Locate and initialize the p-code implementation
    pfile = which('run_Indy_car.p');
    if isempty(pfile)
        error('run_Indy_car.p not found on MATLAB path.');
    end
    fprintf('Part I.j: Using run_Indy_car.p from %s\n', pfile);
    ...
    omega_model_motor = lsim(G_omega_with_L_gear, u_compare, t_compare);
    omega_model_motor = omega_model_motor(:);
    omega_model_out = lsim(G_omega_out_with_L, u_compare, t_compare);
    omega_model_out = omega_model_out(:);

    % Steady-state and time-constant verification (output shaft)
    tail_count = min(steps, max(5, round(0.1 * steps)));
    omega_model_ss = mean(omega_model_out(end-tail_count+1:end));
    ...
    idx_tau_model = find(omega_model_out >= tau_target_model, 1, 'first');
    ...
    rmse_compare = sqrt(mean((omega_model_out - omega_output_pcode).^2));
    ...
    plot(t_compare, omega_model_out, 'LineWidth', 1.5); hold on;
    ...
    grid on;
    title('Motor Speed Benchmark');
    xlabel('Time [s]');
    ylabel('\\omega_{m} [rad/s]');
    legend('Analytical motor model','run\\_Indy\\_car.p data','Location','southeast');

    % Store comparison results
    results_part1.gearbox_comparison = table(t_compare, omega_model_out, omega_output_pcode, ...
        'VariableNames', {'Time_s','Omega_model_rad_s','Omega_pcode_rad_s'});
